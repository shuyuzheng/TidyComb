...
#' Chain all calculatrion about one dose-response matrix together (sensitivity
...
#'  }
#' @export
NoCorCalculateMat <- function(response.mat, correction = TRUE, ...) {
  ...
  # 1.3. Correct baseline to 0 if "correction" is TRUE
  if (correction) {
    response.mat.cor <- CorrectBaseLine(response.mat)
  }
  # 2. Single drug process
  # 2.1. Fit single drug dose-response curve
  # drug_col
  ...
  # 3. whole matrix process
  # 3.1 Calculate synergyscores
  zip <- CalculateZIP(response.mat.cor)
  loewe <- CalculateLoewe(response.mat.cor)
  hsa <- CalculateHSA(response.mat.cor)
  bliss <- CalculateBliss(response.mat.cor)
...
#' Calculate Drug Combination data in template format (sensitivity without
#' baseline correction)
#'
#' @param template a dataframe in the format as template. Columns "block_id",
#' "drug_row", "drug_col", "response", "conc_r", "conc_c", "conc_r_unit",
...
#'
#' @export
NoCorCalculateTemplate <- function(template, ...) {
  if (!all(c("block_id", "drug_row", "drug_col", "response", "conc_r", "conc_c",
             "conc_r_unit", "conc_c_unit","cell_line_name", "drug_row",
             "drug_col") %in%
           colnames(template)))
  ...
  for (block in blocks) {
    ...
    # 2. Do calculation on matrix
    tmp <- NoCorCalculateMat(response.mat = response.mat)
    tmp <- lapply(tmp, function(x){
      x$block_id = rep(block, nrow(x))
      return(x)
    })

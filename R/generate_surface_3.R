...
# c("block_id", "conc_r","conc_c", "response", "drug_row_id", "drug_col_id",
#   "conc_r_unit", "conc_c_unit", "cell_line_id", "row", "col", "synergy_zip",
...
GenerateSurface3 <- function(response_with_scores) {
  ...
  for(i in 1:len) {
    ...
    mat2$drug_row_id <- data2$drug.pairs$drug.row[index]
    mat2$drug_col_id <- data2$drug.pairs$drug.col[index]

...
GenerateCurve <- function(response_with_scores) {
  ...
  for(i in 1:len){
    ...
    coef.row <- tryCatch ({
      tmp <- drc::drm(single.row$response ~ single.row$conc_r, fct = LL.4(),
      ...
      tmp <- drc::drm(single.row$response ~ log(single.row$conc_r), fct = L.4(),
                      control = drmc(errorm = FALSE, noMessage = TRUE))
    }, error = function(cond) {
      tmp <- drc::drm(single.row$response ~ log(single.row$conc_r), fct = L.4(),
                      control = drmc(errorm = FALSE, noMessage = TRUE))
  ...
  coef.col = tryCatch ({
    tmp = drc::drm(single.col$response ~ single.col$conc_c, fct = LL.4(),
                   control = drmc(errorm = FALSE, noMessage = TRUE))
    }, warning = function(cond){
      tmp = drc::drm(single.col$response ~ log(single.col$conc_c), fct = L.4(),
                     control = drmc(errorm = FALSE, noMessage = TRUE))
    }, error = function(cond){
      tmp = drc::drm(single.col$response ~ log(single.col$conc_c), fct = L.4(),
                     control = drmc(errorm = FALSE, noMessage = TRUE))
  ...
  mat6 <- setNames(mat6, c("block_id","drug_row","drug_col","conc_r_unit",
                           "conc_c_unit","b","c","d","e","model"))

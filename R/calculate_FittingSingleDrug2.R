FittingSingleDrug2 <- function (response.mat,
  ...
  drug.col.model <- tryCatch({
    drc::drm(effect ~ conc, data = drug.col, fct = LL.4(fixed = fixed),
        na.action = na.omit, control = drmc(errorm = FALSE, noMessage = TRUE))
  }, warning = function(w) {
    drc::drm(effect ~ log(conc), data = drug.col, fct = L.4(fixed = fixed),
        na.action = na.omit, control = drmc(errorm = FALSE, noMessage = TRUE))
  }, error = function(e) {
    drc::drm(effect ~ log(conc), data = drug.col, fct = L.4(fixed = fixed),
        na.action = na.omit, control = drmc(errorm = FALSE, noMessage = TRUE))
  ...
  drug.row.model <- tryCatch({
    drc::drm(effect ~ conc, data = drug.row,
             fct = LL.4(fixed = fixed),
             control = drmc(errorm = F, noMessage = T))
  }, warning = function(w) {
